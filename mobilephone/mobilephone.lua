...
--get message content and sender
    ...
    else chat[chatcount] = {message, ''} end
...
function UpdateSong()
    ...
    elseif not ac.currentlyPlaying().isPlaying and not data.nowplaying.isPaused and settings.nowplaying then
        if scrlintvl then clearInterval(scrlintvl) scrlintvl = nil end
        ...
        ui.tabItem('Settings', function()
            ...
            --Display Duration and Forces Sliders
             if settings.damage then
                ui.text('\t')
                ui.sameLine()
                settings.damageduration, damageChange = ui.slider('##Duration', settings.damageduration, 1,60, 'Duration: '..'%.1f')
                if damageChange then damageDurationTimer = settings.damageduration end
                ui.text('\t')
                ui.sameLine()
                settings.fadeduration, fadeChange = ui.slider('##Fadeout', settings.fadeduration, 1,60, 'Fade out: '..'%.1f')
